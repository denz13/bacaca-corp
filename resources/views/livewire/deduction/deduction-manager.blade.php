<div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-foreground/5 border-b border-foreground/10">
                    <tr>
                        <th class="p-3 text-left font-medium cursor-pointer select-none" wire:click="sortBy('description')">
                            Description
                            @if($sortField === 'description')
                                <span class="ml-1 opacity-60">{{ $sortDirection === 'asc' ? '▲' : '▼' }}</span>
                            @endif
                        </th>
                        <th class="p-3 text-left font-medium cursor-pointer select-none" wire:click="sortBy('amount')">
                            Amount
                            @if($sortField === 'amount')
                                <span class="ml-1 opacity-60">{{ $sortDirection === 'asc' ? '▲' : '▼' }}</span>
                            @endif
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="p-4 text-center opacity-70">No deductions found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <x-menu.modal :showButton="false" modalId="create-deduction" title="Add Deduction" description="Create a new deduction" size="md" :isOpen="$showCreateModal">
        <div class="space-y-4">
            <div>
                <label class="text-sm font-medium">Description</label>
                <input type="text" wire:model.defer="newDescription" class="w-full mt-1 h-10 rounded-md border bg-background px-3 py-2" placeholder="Enter description">
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" wire:model.defer="newAmount" class="w-full mt-1 h-10 rounded-md border bg-background px-3 py-2" placeholder="0.00">
                @error('newAmount')
                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                @enderror
            </div>
        ...
        <x-slot:footer>
            ...
            <button wire:click="saveDeduction" type="button" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">Save</button>
        </x-slot:footer>
    </x-menu.modal>

    <x-menu.modal :showButton="false" modalId="edit-deduction" title="Update Deduction" description="Edit existing deduction" size="md" :isOpen="$showEditModal">
        <div class="space-y-4">
            <div>
                <label class="text-sm font-medium">Description</label>
                <input type="text" wire:model.defer="editDescription" class="w-full mt-1 h-10 rounded-md border bg-background px-3 py-2" placeholder="Enter description">
            ...
            <div>
                <label class="text-sm font-medium">Amount</label>
                <input type="number" step="0.01" min="0" wire:model.defer="editAmount" class="w-full mt-1 h-10 rounded-md border bg-background px-3 py-2" placeholder="0.00">
                @error('editAmount')
                    <div class="text-danger text-xs mt-1">{{ $message }}</div>
                @enderror
            </div>
        ...
        <x-slot:footer>
            ...
            <button wire:click="updateDeduction" type="button" class="cursor-pointer inline-flex border items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-(--color)/20 border-(--color)/60 text-(--color) hover:bg-(--color)/5 [--color:var(--color-primary)] h-10 px-4 py-2 w-24">Update</button>
        </x-slot:footer>
    </x-menu.modal>

    <x-menu.modal :showButton="false" modalId="success-deduction" title="Success" description="Action completed" size="sm" :isOpen="$showSuccessModal">
        <div class="space-y-2">
